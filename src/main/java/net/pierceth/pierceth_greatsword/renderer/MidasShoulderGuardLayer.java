...
public class MidasShoulderGuardLayer<T extends LivingEntity, M extends HumanoidModel<T>> extends RenderLayer<T, M> {
    private static final ResourceLocation TEXTURE = new ResourceLocation(PiercethGreatsword.MODID,"textures/item/midas_shoulder_guard.png");
    ...
    public MidasShoulderGuardLayer(RenderLayerParent<T, M> pRenderer) {
        super(pRenderer);
        this.shoulderGuardModel = new MidasShoulderGuardTwoModel<>(Minecraft.getInstance().getEntityModels().bakeLayer(MidasShoulderGuardTwoModel.MIDAS_SHOULDER_GUARD));
    }
    ...
    public boolean shouldRender(ItemStack itemStack, T entity) {
        return itemStack.is(PierceTHItems.MIDAS_GAUNTLET.get());
    }
